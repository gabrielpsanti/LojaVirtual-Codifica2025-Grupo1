...
@section('titulo-aba', 'Tamanhos')
@section('titulo-header', 'Tamanhos')
...
@section('content')
    <section class="w-[60vw] mx-auto flex flex-col gap-4">
        <div class="flex flex-col rounded-3xl bg-white px-6 py-4 shadow-sm flex-row items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold text-slate-800">Gerenciamento de tamanhos</h2>
                <p class="text-sm text-slate-500">Cadastre, edite e remova os tamanhos disponíveis no sistema.</p>
            </div>

            <a href="{{ route('admin.tamanhos.create') }}"
        ...
        <div class="rounded-3xl bg-white px-6 py-5 shadow-sm">
            <form method="GET" action="{{ route('admin.tamanhos.index') }}" class="flex flex-col gap-4">
                <div>
                    <label for="busca" class="mb-1 text-sm font-medium text-slate-700">Pesquisar tamanho</label>
            ...
            <table class="min-w-full divide-y divide-slate-200">
                <thead class="bg-slate-900 text-slate-200 text-left text-xs font-semibold">
                    <tr class="divide-slate-200">
                        ...
                        <th class="px-6 py-4 text-right pr-[5.5rem]">AÇÕES
                        </th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-300">
                    @forelse ($tamanhos as $tamanho)
                        <tr class="hover:bg-slate-100">
                            <td class="px-6 py-4 text-sm text-slate-500">{{ $tamanho->id_tamanho }}</td>
                            <td class="px-6 py-4 text-md font-medium text-slate-800">{{ $tamanho->nome }}</td>
                            <td class="px-6 py-4">
                                <div class="flex justify-end gap-2">
                                    <a href="{{ route('admin.tamanhos.edit', $tamanho) }}"
                                    ...
                                    <form action="{{ route('admin.tamanhos.destroy', $tamanho) }}" method="POST"
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="px-6 py-10 text-center text-lg text-slate-500">
                                Nenhum tamanho cadastrado até o momento.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
